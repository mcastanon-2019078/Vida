...
export const get = async (req, res) => {
    try {
        ...
        if (alertas.length === 0) return res.status(404).send({ message: 'There are no alerts to see' });
...
export const deleteAlert = async (req, res) => {
    try {
        ...
        if (deletedAlert.deletedCount == 0) return res.status(404).send({ message: 'Alert not found and not deleted' });
