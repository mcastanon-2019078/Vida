...
import { useDesactivarAlerta } from '../../hooks/useDesactiveAlert.jsx';

const AlertAdmin = () => {
  const { alerts, isFetching, getAlerts } = useGetAlertas();
  const { isLoading, addAlertaClient } = useAlertaClient();
  const { desactivar, mensaje } = useDesactivarAlerta()
  ...
  const handleDesactivarClick = async (alertId) => {
    try {
      console.log('Intentando desactivar la alerta con ID:', alertId)

      const alertToDesactivar = alertList.find(item => item._id === alertId)
      if (!alertToDesactivar) {
        console.error('La alerta con ID', alertId, 'no existe en la lista actual.')
        return
      }

      await desactivar(alertId)
      setSelectedAlert(null)
    } catch (error) {
          ...
          {alertList.map((alert) => (
            ...
            >
              ...
              <View style={styles.actionButtons}>
                ...
                <Button title="Desactivar" onPress={() => handleDesactivarClick(alert._id)} />
              </View>
            </TouchableOpacity>
          ))}
        </ScrollView>
            ...
            {selectedAlert && (
              <View style={styles.cardSelected}>
                ...
                <View style={styles.actionButtons}>
                  ...
                  <Button title="Desactivar" onPress={() => handleDesactivarClick(selectedAlert._id)} />
                </View>
              </View>
            )}
          </TouchableOpacity>
