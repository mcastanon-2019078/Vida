    ...
    useEffect(() => {
        const fetchAlerts = async () => {
            try {
                ...
                if (response && response.data && Array.isArray(response.data.alertas)) {
                    console.log("Datos de alerta recibidos:", response.data.alertas);
                ...
                alerts.map(async (alerta) => {
                    try {
                        ...
                        if (response.data.status === "OK") {
                            ...
                            console.error(`Geocoding error for address ${alerta.direccionDesaparicion}: ${response.data.status}`);
                            return null;
                        }
                    } catch (error) {
                        console.error(`Error fetching coordinates for address ${alerta.direccionDesaparicion}:`, error);
                        return null;
                    }
                })
            );
